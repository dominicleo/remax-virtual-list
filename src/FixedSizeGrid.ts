import createGridComponent, { Props, ScrollToAlign } from './createGridComponent';
...
const FixedSizeGrid = createGridComponent({
  ...
  getOffsetForColumnAndAlignment: (
    ...
    const minOffset = Math.max(
      0,
      columnIndex * (columnWidth as number) - width + scrollbarSize + (columnWidth as number),
    );
  ...
  getOffsetForRowAndAlignment: (
    ...
    const minOffset = Math.max(
      0,
      rowIndex * (rowHeight as number) - height + scrollbarSize + (rowHeight as number),
    );
